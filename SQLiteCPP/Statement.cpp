...
int Statement::initParams(Database* pDatabase, const std::string& aQuery)
{
	int retVal = 0;
	resetParams();

	mQuery = aQuery;
    mpSQLite = pDatabase->getHandle(),
    mpPreparedStatement = prepareStatement();
	mColumnCount = sqlite3_column_count(mpPreparedStatement.get());

	return retVal;
}

void Statement::resetParams()
{
	mQuery = std::string("");
	mColumnCount = 0;
	mbHasRow = false;
...
bool Statement::isColumnExists(const char* apName) const
{
	if (mColumnNames.empty())
	{
		for (int i = 0; i < mColumnCount; ++i)
