...
#include "Exception.h"
#include "Statement.h"
#include "../sqlite3/x64/include/sqlite3.h"
#include <fstream>
#include <string.h>
...
int Database::initV2(const std::string& dbFile, std::string& strErr, const int flag /*= SQLite::OPEN_READONLY*/, const int busyTimeOut /*= 0*/, const std::string& vfs /*= ""*/)
{
	int retVal = 0;

	mFilename = dbFile;
	sqlite3* handle = nullptr;
	const int ret = sqlite3_open_v2(dbFile.c_str(), &handle, flag, vfs.empty() ? nullptr : vfs.c_str());
	mSQLitePtr.reset(handle);
...
int Database::init(const std::string& dbFile, std::string& strErr, const int flag /*= SQLite::OPEN_READONLY*/)
{
	int ret_val = 0;
	...
	sqlite3* handle = nullptr;
	const int ret = sqlite3_open(dbFile.c_str(), &handle);
	mSQLitePtr.reset(handle);
...
bool Database::tableColumnExists(const char* apTableName, const char* apColumnName)
{
	std::string sql = u8"SELECT * FROM ";
	sql.append(apTableName);
	sql.append(u8" LIMIT 0");
	Statement query(*this, sql.c_str());
	(void)query.executeStep(); 
	return query.isColumnExists(apColumnName);
}
